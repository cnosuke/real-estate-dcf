...
// フォーム編集状態
const isFormEditingAtom = atom(false)
const lastModifiedFieldAtom = atom<string | null>(null)

// フォーム操作アクション
export const toggleSectionAtom = atom(null, (get, set, sectionName: string) => {
  switch (sectionName) {
    case 'propertyBasic':
      set(
...
// セクション展開状態の一括管理
const allSectionsExpandedAtom = atom((get) => {
  return (
    get(propertyBasicSectionExpandedAtom) &&
