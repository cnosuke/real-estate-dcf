...
export const dcfCalculationStateAtom = atom<DCFCalculationState>(DCFCalculationState.IDLE)
export const dcfCalculationErrorAtom = atom<DCFError | null>(null)

// DCF計算実行atom（副作用あり）
export const executeDCFCalculationAtom = atom(
  null,
  async (get, set) => {
    ...
    try {
      ...
      const dcfError = error instanceof DCFError 
        ? error 
        : DCFErrorFactory.createCalculationError('dcf_execution', { originalError: error })
...
// 計算状態の便利なselector
export const isCalculatingAtom = atom((get) => 
  get(dcfCalculationStateAtom) === DCFCalculationState.CALCULATING
)

export const hasCalculationErrorAtom = atom((get) => 
  get(dcfCalculationStateAtom) === DCFCalculationState.ERROR
)

export const calculationSuccessAtom = atom((get) => 
  get(dcfCalculationStateAtom) === DCFCalculationState.SUCCESS
)
...
export const dcfResultStateAtom = atom((get) => {
