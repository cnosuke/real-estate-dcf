...
export class GridSearchIRRStrategy implements IRRCalculationStrategy {
  ...
  private coarseGridSearch(cashFlows: number[]): {
    rate: number
    error: number
  } {
    const config = CALCULATION_CONFIG.GRID_SEARCH
  ...
  private fineGridSearch(
    ...
    let best = {
      rate: centerRate,
      error: Math.abs(this.calculateNPV(cashFlows, centerRate)),
    }

    for (
      let offset = -config.FINE_RANGE;
      offset <= config.FINE_RANGE;
      offset += config.FINE_STEP
    ) {
